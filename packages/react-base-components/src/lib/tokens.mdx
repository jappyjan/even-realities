...
export const spacing = [0, 8, 12, 16, 20, 24, 28, 32];

<Meta title="Foundations/Tokens" />

<Title>Tokens / Guidelines</Title>
<Subtitle>Design system tokens for Even OS</Subtitle>
This page lists the foundational tokens used in the component library, derived
from the Figma foundations. Use these utilities and variables to keep spacing,
...
<div className="tokens-typography" style={{ display: 'grid', gap: '12px' }}>
  {typography.map((item) => (
    <div
      key={item.name}
      style={{
...
```tsx
<div className="bg-bc-1 text-tc-1">
...
<div style={{ display: 'grid', gap: '12px' }}>
  {spacing.map((value) => (
    <div
      key={value}
      style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '16px' }}
    >
      <code>{value}px</code>
      <div
        style={{
...
<div style={{ marginTop: '16px', display: 'grid', gap: '8px' }}>
  <div>
    <code>--layout-page-padding</code>: <code>var(--layout-page-padding)</code>
  </div>
  <div>
    <code>--layout-section-gap</code>: <code>var(--layout-section-gap)</code>
  </div>
  <div>
    <code>--layout-card-margin</code>: <code>var(--layout-card-margin)</code>
  </div>
</div>
